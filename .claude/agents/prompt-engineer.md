...
covering `.claude/agents/*.md` files, `@function_tool` docstrings, and
...
```
Before doing anything else, read in this order:
1. `.claude/skills/recall/SKILL.md`
...
Then run `/recall` to write a checkpoint following `.claude/skills/recall/SKILL.md`.
...
Invoke when:
- Creating a new agent file under `.claude/agents/`
